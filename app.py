...
class Planetoids(GameApp):
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE: 
            self._title = GLabel(text="Planetoids",font_name=TITLE_FONT,
                font_size=TITLE_SIZE*.7,left=GAME_WIDTH*.25,
                bottom=GAME_HEIGHT*.6)
            self._message = GLabel(text="Press 's' to start",
                font_name=MESSAGE_FONT,font_size=MESSAGE_SIZE*.7,
                left=GAME_WIDTH*.25,bottom=GAME_HEIGHT*.5)
        if self._state !=STATE_INACTIVE: 
            self._title == None
    ...
    def update(self,dt):
        ...
        if self._state == STATE_INACTIVE: 
            if self.input.is_key_down('s') == True: 
                ...
                jsonfile = self.load_json(DEFAULT_WAVE)
                self._wave = Wave(jsonfile)
    ...
    def draw(self):
        ...
        elif self._state == STATE_ACTIVE and self._wave.getShip() != None:
    ...
    def levels(self): 
        """
        checks if the player has won/died/lost while the game is ongoing
        iterates the number of lives remaining
        """
        if self._state == STATE_ACTIVE: 
    ...
    def paused(self): 
        """
        Draws a paused screen when the ship dies but the player still has lives
        """
        pause1 = GLabel(text="You Died",font_name=TITLE_FONT,
            font_size=TITLE_SIZE*.7,left=GAME_WIDTH*.5,bottom=GAME_HEIGHT*.6)
        pause2 = GLabel(text='You have ' + 
            str(self._wave.getLives()) + 
            " life/lives left. Press 's' to continue",font_name=MESSAGE_FONT,
            font_size=MESSAGE_SIZE*.4,left=GAME_WIDTH*.5,bottom=GAME_HEIGHT*.5)
        pause1.draw(self.view)
        pause2.draw(self.view)
        
    def end(self): 
        """
        Draws an ending screen and message when the player loses all 3 lives
        """
        end1 = GLabel(text="Game Over. You Lost",font_name=TITLE_FONT,
                      font_size=TITLE_SIZE*.5,left=GAME_WIDTH*.5,
                      bottom=GAME_HEIGHT*.6)
        end1.draw(self.view)
        
    def endwin(self): 
        """
        Draws an ending screen and message when the player wins the game by 
        shooting all the asteroids
        """
        endw = GLabel(text="Congrats! You Won",font_name=TITLE_FONT,
                      font_size=TITLE_SIZE*.5,left=GAME_WIDTH*.5,
                      bottom=GAME_HEIGHT*.6)
        endw.draw(self.view)
